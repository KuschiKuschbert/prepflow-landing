...
async function optimizeImage(inputPath) {
  try {
    ...
    console.log(`   Original: ${width}x${height} ${format.toUpperCase()} (${(inputSize / 1024).toFixed(1)}KB)`);
    ...
    console.log(`   WebP: ${width}x${height} (${(webpSize / 1024).toFixed(1)}KB) - ${webpSavings}% smaller`);
    ...
    console.log(`   AVIF: ${width}x${height} (${(avifSize / 1024).toFixed(1)}KB) - ${avifSavings}% smaller`);
...
async function optimizeAllImages() {
  ...
  console.log(`   WebP total: ${(totalWebp / 1024 / 1024).toFixed(2)}MB (${((totalOriginal - totalWebp) / totalOriginal * 100).toFixed(1)}% savings)`);
  console.log(`   AVIF total: ${(totalAvif / 1024 / 1024).toFixed(2)}MB (${((totalOriginal - totalAvif) / totalOriginal * 100).toFixed(1)}% savings)`);
