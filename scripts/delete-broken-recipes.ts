...
async function main() {
  ...
  while (true) {
    ...
    for (const r of batch) {
      ...
      const badIngredients = r.ingredients.filter(
        (i: { unit?: string; quantity?: number | string }) => {
    ...
    process.stdout.write(
      `\rScanning... Verified: ${checkedCount}/${total} | Marked for Deletion: ${allIdsToDelete.length}`,
    );
  ...
  for (let i = 0; i < allIdsToDelete.length; i += DELETE_BATCH) {
    ...
    const { error: delError } = await supabase.from('ai_specials').delete().in('id', batchIds);
