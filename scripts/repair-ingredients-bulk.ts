    ...
    if (jsonMatch) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length === recipes.length) {
          ...
          console.error(
            `Mismatch: Expected ${recipes.length} recipes, got ${Array.isArray(parsed) ? parsed.length : 'invalid type'}`,
          );
...
async function main() {
  ...
  while (hasMore) {
    ...
    const brokenList = specials
      .filter(s => {
        if (!Array.isArray(s.ingredients)) return false;
        const brokenCount = s.ingredients.filter(
          (i: { name?: string; unit?: string; quantity?: number }) => {
      ...
      .map(s => ({
        ...
        ingredientNames: Array.isArray(s.ingredients)
          ? s.ingredients
    ...
    for (let i = 0; i < brokenList.length; i += BATCH_SIZE_AI) {
      ...
      console.log(
        `Processing AI Batch ${Math.floor(i / BATCH_SIZE_AI) + 1} (${chunk.length} recipes)...`,
      );
      ...
      if (results) {
        ...
        for (let j = 0; j < chunk.length; j++) {
          ...
          const cleanedIngredients = rawIngredients.map(
            (p: { name: string; quantity: number | string; unit?: string }) => {
