/**
 * Delish Scraper
 * Scrapes recipes from Delish using JSON-LD structured data
 */
...
export class DelishScraper extends BaseScraper {
  ...
  protected parseRecipe(html: string, url: string): Partial<ScrapedRecipe> | null {
    try {
      ...
      scraperLogger.error('Error parsing Delish recipe:', error);
      return null;
    }
  }
  ...
  async getAllRecipeUrls(): Promise<string[]> {
    ...
    try {
      scraperLogger.info('Attempting to discover Delish URLs via sitemap...');
      const sitemapParser = new SitemapParser();
      const sitemapUrls = await sitemapParser.discoverRecipeUrls('delish');
    ...
    scraperLogger.info('Falling back to pagination crawling for Delish...');
    try {
      const baseUrl = 'https://www.delish.com/recipes';
      let currentPage = 1;
        ...
        const recipeLinks = $('a[href*="/recipes/"]')
          .map((_, el) => {
            ...
            if (href.startsWith('http://') || href.startsWith('https://')) {
              ...
              normalizedUrl = `https://www.delish.com${href}`;
            } else if (href.startsWith('//')) {
            ...
            // Match pattern: /recipes/[recipe-id]/[recipe-slug]
