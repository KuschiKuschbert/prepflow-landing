 ...
 * Scrapes recipes from Simply Recipes using JSON-LD structured data
 */
...
export class SimplyRecipesScraper extends BaseScraper {
  ...
  protected parseRecipe(html: string, url: string): Partial<ScrapedRecipe> | null {
    try {
      ...
      scraperLogger.error('Error parsing Simply Recipes recipe:', error);
      return null;
    }
  }
  ...
  private parseFromHTML($: cheerio.CheerioAPI, url: string): Partial<ScrapedRecipe> | null {
    try {
      const recipe: Partial<ScrapedRecipe> = {
        ...
        rating: this.parseRating($('.rating, .star-rating').attr('data-rating')),
      };
  ...
  async getAllRecipeUrls(): Promise<string[]> {
    ...
    try {
      scraperLogger.info('Attempting to discover Simply Recipes URLs via sitemap...');
      const sitemapParser = new SitemapParser();
      const sitemapUrls = await sitemapParser.discoverRecipeUrls('simply-recipes');
    ...
    scraperLogger.info('Falling back to pagination crawling for Simply Recipes...');
    try {
      const baseUrl = 'https://www.simplyrecipes.com/recipes';
      let currentPage = 1;
      ...
      while (hasMorePages) {
        ...
        // Extract recipe links matching pattern: /recipes/[recipe-slug]
        const recipeLinks = $('a[href*="/recipes/"]')
          .map((_, el) => {
            ...
            if (href.startsWith('http://') || href.startsWith('https://')) {
              ...
              normalizedUrl = `https://www.simplyrecipes.com${href}`;
            } else if (href.startsWith('//')) {
            ...
            if (cleanUrl.match(/\/recipes\/[^/]+$/) && !cleanUrl.includes('/recipes/collection/')) {
              return cleanUrl;
            }
            return null;
          })
    ...
    scraperLogger.info(`Total Simply Recipes URLs discovered: ${urls.length}`);
    return urls;
  }
}
