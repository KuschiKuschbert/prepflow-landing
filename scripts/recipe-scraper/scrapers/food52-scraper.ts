/**
 * Food52 Scraper
 * Scrapes recipes from Food52 using JSON-LD structured data
 */
...
export class Food52Scraper extends BaseScraper {
  ...
  protected parseRecipe(html: string, url: string): Partial<ScrapedRecipe> | null {
    try {
      ...
      scraperLogger.error('Error parsing Food52 recipe:', error);
      return null;
    }
  }
  ...
  async getAllRecipeUrls(): Promise<string[]> {
    ...
    try {
      scraperLogger.info('Attempting to discover Food52 URLs via sitemap...');
      const sitemapParser = new SitemapParser();
      const sitemapUrls = await sitemapParser.discoverRecipeUrls('food52');
    ...
    scraperLogger.info('Falling back to pagination crawling for Food52...');
    try {
      const baseUrl = 'https://food52.com/recipes';
      let currentPage = 1;
      ...
      while (hasMorePages) {
        ...
        // Extract recipe links matching pattern: /recipes/[recipe-id]-[recipe-slug]
        const recipeLinks = $('a[href*="/recipes/"]')
          .map((_, el) => {
            ...
            if (href.startsWith('http://') || href.startsWith('https://')) {
              ...
              normalizedUrl = `https://food52.com${href}`;
            } else if (href.startsWith('//')) {
    ...
    scraperLogger.info(`Total Food52 URLs discovered: ${urls.length}`);
    return urls;
  }
}
