 ...
 * Scrapes recipes from Serious Eats using JSON-LD structured data
 */
...
export class SeriousEatsScraper extends BaseScraper {
  ...
  protected parseRecipe(html: string, url: string): Partial<ScrapedRecipe> | null {
    try {
      ...
      const recipe: Partial<ScrapedRecipe> = {
        ...
        // Serious Eats doesn't have user ratings (professional/editorial site)
      ...
      scraperLogger.error('Error parsing Serious Eats recipe:', error);
      return null;
    }
  }
  ...
  async getAllRecipeUrls(): Promise<string[]> {
    ...
    try {
      scraperLogger.info('Attempting to discover Serious Eats URLs via sitemap...');
      const sitemapParser = new SitemapParser();
      const sitemapUrls = await sitemapParser.discoverRecipeUrls('serious-eats');
    ...
    scraperLogger.info('Falling back to pagination crawling for Serious Eats...');
    try {
      const baseUrl = 'https://www.seriouseats.com/recipes';
      let currentPage = 1;
        ...
        const recipeLinks = $('a[href*="/recipes/"]')
          .map((_, el) => {
            ...
            if (href.startsWith('http://') || href.startsWith('https://')) {
              ...
              normalizedUrl = `https://www.seriouseats.com${href}`;
            } else if (href.startsWith('//')) {
    ...
    scraperLogger.info(`Total Serious Eats URLs discovered: ${urls.length}`);
    return urls;
  }
}
