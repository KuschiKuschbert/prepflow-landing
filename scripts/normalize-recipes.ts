...
async function main() {
  ...
  console.log(
    '--- Processing ingredients table (skipping for now based on previous clean run) ---',
  );
  /*
  ...
  */
  ...
  while (hasMore) {
    ...
    for (const special of specials) {
      ...
      const newIngredients = special.ingredients.map(
        (
          ing: string | { name?: string; unit?: string; quantity?: number; original_text?: string },
        ) => {
  ...
  console.log(
    `Completed specials. ${specialUpdates} recipes ${DRY_RUN ? 'would be' : 'were'} updated out of ${totalProcessed} scanned.`,
  );
