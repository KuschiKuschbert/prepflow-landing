...
export function getBlurDataURL(aspectRatio: string): string {
  return (
    IMAGE_CONFIG.blurDataURLs[aspectRatio as keyof typeof IMAGE_CONFIG.blurDataURLs] ||
    IMAGE_CONFIG.blurDataURLs['16:9']
  );
}
...
export const customImageLoader: ImageLoader = ({ src, width, quality = 85 }) => {
  ...
  const optimizedQuality =
    IMAGE_CONFIG.quality[imageType as keyof typeof IMAGE_CONFIG.quality] ||
    IMAGE_CONFIG.quality.content;
...
export class ImageOptimizer {
  ...
  preloadImage(src: string, imageType: string = 'content'): void {
    ...
    link.href = customImageLoader({
      src,
      width: 800,
      quality: IMAGE_CONFIG.quality[imageType as keyof typeof IMAGE_CONFIG.quality],
    });
...
export function detectImageFormat(
  src: string,
): 'avif' | 'webp' | 'jpeg' | 'png' | 'svg' | 'unknown' {
