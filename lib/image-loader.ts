...
export function generateImageSrcSet(src: string, widths: number[] = [640, 750, 828, 1080, 1200, 1920]): string {
  const imageType = getImageType(src);
...
export function preloadCriticalImages(): void {
  ...
  criticalImages.forEach(({ src, type }) => {
    ...
    link.rel = 'preload';
    link.as = 'image';
    link.href = customImageLoader({ src, width: 800, quality: IMAGE_CONFIG.quality[type as keyof typeof IMAGE_CONFIG.quality] });
