/**
 * Simply Recipes Scraper (Migrated from scripts)
 */
...
export class SimplyRecipesScraper extends BaseScraper {
  ...
  async getRecipeUrls(limit: number = 50): Promise<string[]> {
    ...
    const urls = await sitemapParser.discoverRecipeUrls('simply-recipes');
    return urls.slice(0, limit);
  }

  async getAllRecipeUrls(): Promise<string[]> {
    const sitemapParser = new SitemapParser();
    return sitemapParser.discoverRecipeUrls('simply-recipes');
  }
}
