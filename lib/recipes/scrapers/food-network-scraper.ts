/**
 * Food Network Scraper (Migrated from scripts)
 */
...
export class FoodNetworkScraper extends BaseScraper {
  ...
  protected async fetchPageWithPuppeteer(url: string): Promise<string> {
    if (!this.usePuppeteer) return this.fetchPage(url);
    ...
    try {
      ...
      scraperLogger.error(`Puppeteer fetch failed for ${url}:`, error);
  ...
  async getRecipeUrls(limit: number = 50): Promise<string[]> {
    ...
    const urls = await sitemapParser.discoverRecipeUrls('food-network');
    return urls.slice(0, limit);
  }

  async getAllRecipeUrls(): Promise<string[]> {
    const sitemapParser = new SitemapParser();
    return sitemapParser.discoverRecipeUrls('food-network');
  }
}
