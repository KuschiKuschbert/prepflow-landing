/**
 * BBC Good Food Scraper (Migrated from scripts)
 */
...
export class BBCGoodFoodScraper extends BaseScraper {
  ...
  async getRecipeUrls(limit: number = 50): Promise<string[]> {
    ...
    const urls = await sitemapParser.discoverRecipeUrls('bbc-good-food');
    return urls.slice(0, limit);
  }

  async getAllRecipeUrls(): Promise<string[]> {
    const sitemapParser = new SitemapParser();
    return sitemapParser.discoverRecipeUrls('bbc-good-food');
  }
}
