/**
 * The Kitchn Scraper (Migrated from scripts)
 */
...
export class TheKitchnScraper extends BaseScraper {
  ...
  async getRecipeUrls(limit: number = 50): Promise<string[]> {
    ...
    const urls = await sitemapParser.discoverRecipeUrls('the-kitchn');
    return urls.slice(0, limit);
  }

  async getAllRecipeUrls(): Promise<string[]> {
    const sitemapParser = new SitemapParser();
    return sitemapParser.discoverRecipeUrls('the-kitchn');
  }
}
