/**
 * Serious Eats Scraper (Migrated from scripts)
 */
...
export class SeriousEatsScraper extends BaseScraper {
  ...
  async getRecipeUrls(limit: number = 50): Promise<string[]> {
    ...
    const urls = await sitemapParser.discoverRecipeUrls('serious-eats');
    return urls.slice(0, limit);
  }

  async getAllRecipeUrls(): Promise<string[]> {
    const sitemapParser = new SitemapParser();
    return sitemapParser.discoverRecipeUrls('serious-eats');
  }
}
