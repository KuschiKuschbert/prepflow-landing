...
export function parseIngredientString(rawString: string): ParsedIngredient | null {
  ...
  let processed = clean
      ...
      .replace(/⅕/g, ' 1/5').replace(/⅖/g, ' 2/5')
      .replace(/⅗/g, ' 3/5').replace(/⅘/g, ' 4/5')
  ...
  const packageMatch = processed.match(/^([\d.]+)\s*\(([\d.]+)\s*([a-zA-Z_\-.]+)\)\s*([a-zA-Z_]+)\s+(.+)$/);
  if (packageMatch) {
       const qty = parseFloat(packageMatch[1]);
       const pkgUnit = normalizeUnit(packageMatch[4]);
       if (['can', 'bottle', 'jar', 'pack', 'package', 'bag', 'container', 'box', 'head', 'stick'].includes(pkgUnit)) {
