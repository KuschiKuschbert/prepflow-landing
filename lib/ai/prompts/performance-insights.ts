...
export function buildPerformanceInsightsPrompt(
  ...
  const hiddenGemsList = hiddenGems.slice(0, 10).map(formatItem).join('\n') || 'None';
  const bargainBucketsList = bargainBuckets.slice(0, 10).map(formatItem).join('\n') || 'None';
  ...
  return PERFORMANCE_INSIGHTS_PROMPT_TEMPLATE
    ...
    .replace('{{hiddenGemsCount}}', hiddenGems.length.toString())
    .replace('{{hiddenGemsList}}', hiddenGemsList)
    .replace('{{bargainBucketsCount}}', bargainBuckets.length.toString())
    .replace('{{bargainBucketsList}}', bargainBucketsList)
    .replace('{{burntToastCount}}', burntToast.length.toString())
    .replace('{{burntToastList}}', burntToastList)
    .replace('{{chefsKissCount}}', chefsKiss.length.toString())
    .replace('{{chefsKissList}}', chefsKissList);
}
