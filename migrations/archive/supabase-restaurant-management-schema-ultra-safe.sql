-- PrepFlow Restaurant Management Features Database Schema (Ultra-Safe Version)
-- This extends the existing PrepFlow schema with new restaurant management features
-- Handles existing tables gracefully by adding missing columns
...
DO $$ 
BEGIN
    -- Add is_active column to cleaning_areas if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'cleaning_areas' AND column_name = 'is_active') THEN
        ALTER TABLE cleaning_areas ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
END $$;
...
-- Temperature equipment (fridges, freezers, bain maries)
CREATE TABLE IF NOT EXISTS temperature_equipment (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  name VARCHAR(255) NOT NULL, -- e.g., "Main Fridge", "Freezer 1", "Bain Marie Station"
  equipment_type VARCHAR(50) NOT NULL, -- fridge, freezer, bain_marie, storage
  location VARCHAR(255), -- e.g., "Kitchen", "Storage Room", "Prep Area"
  min_temp_celsius DECIMAL(5,2), -- Recommended minimum temperature
  max_temp_celsius DECIMAL(5,2), -- Recommended maximum temperature
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
DO $$ 
BEGIN
    -- Add is_active column to compliance_types if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'compliance_types' AND column_name = 'is_active') THEN
        ALTER TABLE compliance_types ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
END $$;
...
DO $$ 
BEGIN
    -- Add is_active column to suppliers if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
...
DO $$ 
BEGIN
    -- Add is_active column to kitchen_sections if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'kitchen_sections' AND column_name = 'is_active') THEN
        ALTER TABLE kitchen_sections ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
END $$;
...
DO $$ 
BEGIN
    -- Add is_active column to shared_recipes if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'shared_recipes' AND column_name = 'is_active') THEN
        ALTER TABLE shared_recipes ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
END $$;
...
-- Temperature equipment indexes
CREATE INDEX IF NOT EXISTS idx_temperature_equipment_type ON temperature_equipment(equipment_type);
CREATE INDEX IF NOT EXISTS idx_temperature_equipment_active ON temperature_equipment(is_active);
...
DROP TRIGGER IF EXISTS update_temperature_equipment_updated_at ON temperature_equipment;
...
CREATE TRIGGER update_temperature_equipment_updated_at BEFORE UPDATE ON temperature_equipment FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON temperature_equipment;
...
CREATE POLICY "Allow all operations for authenticated users" ON temperature_equipment FOR ALL TO authenticated USING (true);
...
COMMENT ON TABLE temperature_equipment IS 'Temperature equipment inventory (fridges, freezers, bain maries)';
