      ...
      <section id="features" className="py-12 md:py-16">
        <div className="grid gap-6 md:grid-cols-2">
          <FeatureCard title="Easy COGS">
            Enter ingredients; the sheet handles the math.
          </FeatureCard>
          <FeatureCard title="Yield & Waste Aware">
            Include trim and prep yields for accurate costing.
          </FeatureCard>
          <FeatureCard title="Item-Level Clarity">
            See which items help or hurt your menu mix.
          </FeatureCard>
          <FeatureCard title="GST & Currency">
            Built for AU GST; priced in AUD.
          </FeatureCard>
        </div>
      </section>
...
function DynamicPriceCard() {
  ...
  return (
    <div className="rounded-2xl border border-gray-700 bg-[#121212] p-5 flex flex-col max-w-md">
      ...
      {next && (
        <p className="mt-3 text-xs text-gray-500">Next change: {formatAud(next.price)} on {next.date.toLocaleDateString()} (in {daysUntil(next.date)} days)</p>
      )}
    </div>
  );
}
