...
export default function ScrollTracker({ onSectionView, onScrollDepth, enabled = true }: ScrollTrackerProps) {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        if (scrollDuration > 1000) { // Only track if scrolling for more than 1 second
            ...
            entries.forEach((entry) => {
              if (entry.isIntersecting) {
                ...
                if (!viewedSections.has(sectionId)) {
                  ...
                  // Track engagement
                  trackEngagement('landing_page_variants', 'user_' + Math.random().toString(36).substr(2, 9), 'section_view', {
                  ...
                  if (typeof window !== 'undefined' && window.gtag) {
                    window.gtag('event', 'section_view', {
                      ...
                      custom_parameter_section_name: entry.target.textContent?.substring(0, 50) || 'unknown',
