...
function BarChart({ data, height = 200, width = 400, showValues = true, className = '' }: BarChartProps) {
  const maxValue = Math.max(...data.map(d => d.value));
  const colors = ['#29E7CD', '#3B82F6', '#D925C7', '#10B981', '#F59E0B', '#EF4444'];

  return (
    <div className={`bg-[#1f1f1f] border border-[#2a2a2a] rounded-2xl p-6 ${className}`}>
        ...
        {data.map((item, index) => {
          ...
          return (
            <div key={item.name} className="flex flex-col items-center flex-1">
              ...
              />
              <span className="text-xs text-gray-300 mt-2 text-center leading-tight">
                {item.name}
              </span>
            </div>
          );
...
function LineChart({ data, height = 200, width = 400, showGrid = true, showValues = false, className = '' }: LineChartProps) {
        ...
        {data.map((item, index) => {
          ...
          return (
            <circle
              ...
              r="4"
              fill="#29E7CD"
              className="transition-all duration-300 hover:r-6 hover:fill-[#3B82F6]"
            >
              {showValues && (
