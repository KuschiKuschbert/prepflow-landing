 ...
 * - Large Desktop (1440-1919px): 6 columns
 * - XL (1920-2559px): 8 columns
...
export function SummaryCardGrid({ children, className = '', gap = 'md' }: SummaryCardGridProps) {
  ...
  return (
    <div
      className={`grid grid-cols-1 tablet:grid-cols-2 desktop:grid-cols-3 large-desktop:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-8 ${gapClasses[gap]} ${className}`}
    >
      {children}
    </div>
  );
