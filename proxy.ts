...
// const isDevelopment = process.env.NODE_ENV === 'development'; // Moved inside proxy
// const authBypassDev = process.env.AUTH0_BYPASS_DEV === 'true'; // Moved inside proxy
...
export default async function proxy(req: NextRequest) {
  ...
  if (isPerfTest) {
    logger.warn(`[Proxy V5] !!! BYPASS TRIGGERED !!! for: ${pathname}`);
  ...
  if (isDevelopment && authBypassDev) {
    logger.dev(`[Proxy V5] Dev Auth bypass enabled for: ${pathname}`);
  ...
  // Skip rate limiting when DISABLE_RATE_LIMIT=true (e.g. simulation tests)
  const disableRateLimit = process.env.DISABLE_RATE_LIMIT === 'true';

  if (isApi && !isPublicApi && !isAuthRoute && !isStatusPollingApi && !disableRateLimit) {
  ...
  try {
    ...
    if (pathname.startsWith('/api/auth/')) {
      ...
      logger.dev('[Proxy] Auth0 handled auth route:', {
        pathname,
    ...
    logger.error('[Proxy] Auth0 middleware error:', {
    ...
    if (pathname.startsWith('/api/auth/')) {
      ...
      logger.error('[Proxy] Auth0 auth route failed:', {
        pathname,
        error: errorMessage,
        stack: errorStack,
      });
      ...
      try {
        ...
        Promise.all([
          detectAndStoreUserCountry(session.user.email, req.headers).catch(err => {
            logger.warn('[Proxy] Failed to detect/store country:', {
              error: err instanceof Error ? err.message : String(err),
              email: session.user.email,
            });
          }),
          detectAndStoreEUStatus(session.user.email, req.headers).catch(err => {
            logger.warn('[Proxy] Failed to detect/store EU status:', {
              error: err instanceof Error ? err.message : String(err),
              email: session.user.email,
            });
          }),
        ...
        logger.warn('[Proxy] Error importing detection utilities:', {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    }
