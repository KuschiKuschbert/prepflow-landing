...
export default function PerformanceFilters({
  ...
  return (
    <div
      ref={filterRef}
      className={`mb-3 space-y-2 transition-all tablet:mb-4 tablet:space-y-3 desktop:mb-6 desktop:space-y-4 ${
        isSticky
          ? 'sticky top-0 z-40 rounded-b-xl border-b border-[#2a2a2a] bg-[#1f1f1f] p-3 shadow-lg tablet:p-4 desktop:p-6'
          : ''
      }`}
    >
      {/* Result count */}
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-400">
          ...
          <span className="font-semibold text-white">{performanceItems.length}</span> items
        </div>
        {hasActiveFilters && (
          <button
            onClick={() => {
            ...
            className="text-fluid-sm text-[#29E7CD] hover:text-[#29E7CD]/80 transition-colors"
          >
            Clear All Filters
          </button>
        )}
          ...
          {MENU_ITEM_CLASSES.map(className => {
            ...
            return (
              ...
              >
                {className}
                <span className={`ml-2 ${isActive ? 'opacity-100' : 'opacity-60'}`}>
                  ({count})
                </span>
              </button>
            );
