  ...
  return `
    <div style="max-width: 100%;">
      ...
      <div style="margin-bottom: 32px; padding: 20px; background: rgba(42, 42, 42, 0.3); border-radius: 12px; border: 1px solid rgba(41, 231, 205, 0.2);">
        <h2 style="font-size: 24px; font-weight: 600; color: #ffffff; margin-bottom: 16px; border-bottom: 2px solid rgba(41, 231, 205, 0.3); padding-bottom: 8px;">
          Performance Summary
        </h2>
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px;">
          <div>
            <div style="font-size: 14px; color: rgba(255, 255, 255, 0.7); margin-bottom: 4px;">Total Items</div>
            <div style="font-size: 28px; font-weight: 700; color: #29E7CD;">${items.length}</div>
          </div>
          ${
          ...
          <div>
            ...
            <div style="font-size: 28px; font-weight: 700; color: #29E7CD;">${items.filter(i => i.menu_item_class === "Chef's Kiss").length}</div>
          </div>
          <div>
            <div style="font-size: 14px; color: rgba(255, 255, 255, 0.7); margin-bottom: 4px;">Burnt Toast</div>
            <div style="font-size: 28px; font-weight: 700; color: #D925C7;">${items.filter(i => i.menu_item_class === 'Burnt Toast').length}</div>
          </div>
        </div>
        ${
          ...
          <div style="margin-top: 16px; padding-top: 16px; border-top: 1px solid rgba(42, 42, 42, 0.5);">
            <div style="font-size: 14px; color: rgba(255, 255, 255, 0.7);">
              Date Range: ${dateRange.startDate.toLocaleDateString('en-AU')} - ${dateRange.endDate.toLocaleDateString('en-AU')}
            </div>
          </div>
        `
            : ''
      ...
      ${Object.entries(itemsByClass)
        ...
        <div style="margin-bottom: 32px; page-break-inside: avoid;">
          <h3 style="font-size: 20px; font-weight: 600; color: ${getClassColor(className)}; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid rgba(41, 231, 205, 0.2);">
            ${className}
