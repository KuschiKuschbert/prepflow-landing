...
/**
 * Basic info fields (Kitchen Section, Name, Notes)
 */
function PrepListBasicInfo({
  formData,
  setFormData,
  sections,
}: {
      ...
      <div className="desktop:grid-cols-2 grid grid-cols-1 gap-4">
        <div>
          ...
          <select
            ...
            className="w-full rounded-xl border border-[var(--border)] bg-[var(--muted)] px-4 py-3 text-[var(--foreground)] focus:ring-2 focus:ring-[var(--primary)]"
            required
          >
    ...
    <div className="rounded-xl bg-[var(--muted)]/30 p-4">
      <div className="tablet:grid-cols-3 desktop:grid-cols-4 mb-3 grid grid-cols-1 gap-3">
        <div className="desktop:col-span-2">
          ...
          <select
            ...
            className="w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--foreground)] focus:ring-2 focus:ring-[var(--primary)]"
            required
          >
      ...
      <div className="flex items-center justify-between">
        <input
          ...
          className="mr-3 flex-1 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--foreground)] focus:ring-2 focus:ring-[var(--primary)]"
          placeholder={String(t('prepLists.itemNotes', 'Item notes (optional)'))}
        />
        <button
          type="button"
          onClick={onRemove}
          className="rounded-lg p-2 text-[var(--color-error)] hover:bg-[var(--color-error)]/10"
        >
  ...
  return (
    <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <PrepListBasicInfo formData={formData} setFormData={setFormData} sections={sections} />
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={addItem}
              className="rounded-xl bg-[var(--primary)]/10 px-4 py-2 text-sm font-semibold text-[var(--primary)] hover:bg-[var(--primary)]/20"
            >
        ...
        <div className="flex space-x-3 pt-4">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 rounded-xl bg-[var(--muted)] px-4 py-3 text-[var(--foreground-secondary)] hover:bg-[var(--muted)]/80"
          >
            {t('prepLists.cancel', 'Cancel')}
          </button>
          <button
            type="submit"
            className="flex-1 rounded-xl bg-gradient-to-r from-[var(--primary)] to-[var(--accent)] px-4 py-3 font-semibold text-[var(--button-active-text)] hover:shadow-xl"
          >
