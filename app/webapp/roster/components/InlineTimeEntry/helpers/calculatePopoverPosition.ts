 ...
 */
export function calculatePopoverPosition(cellPosition?: {
  top: number;
  left: number;
  width: number;
  height: number;
  ...
  let top = shouldPlaceAbove
    ? cellPosition.top - popoverHeight - spacing
    : cellPosition.top + cellPosition.height + spacing;
  top = Math.max(
    viewportPadding,
    Math.min(window.innerHeight - popoverHeight - viewportPadding, top),
  );
  let left = cellPosition.left + cellPosition.width / 2 - popoverWidth / 2;
  left = Math.max(
    viewportPadding,
    Math.min(window.innerWidth - popoverWidth - viewportPadding, left),
  );
  return { top, left };
}
