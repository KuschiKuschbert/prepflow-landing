...
export async function performDeleteEmployeeFromRoster({
  ...
  if (employeeShifts.length === 0) {
    if (addedEmployeeIds.has(employeeId)) {
      setAddedEmployeeIds(prev => { const next = new Set(prev); next.delete(employeeId); return next; });
  ...
  employeeShifts.forEach(shift => { removeShift(shift.id); removeValidationWarning(shift.id); });
  ...
  try {
    ...
    if (errors.length > 0) {
      rollbackEmployeeDeletion(originalShifts, wasInAddedEmployees, employeeId, addShift, setAddedEmployeeIds);
