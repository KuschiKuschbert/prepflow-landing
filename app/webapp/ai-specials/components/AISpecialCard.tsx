      ...
      <div className="mb-4 flex items-start space-x-4">
        ...
        <div className="flex-1">
          <div className="mb-3 flex items-center space-x-4">
            <div>
              <h3 className="text-lg font-semibold text-white">{t('aiSpecials.aiAnalysis', 'AI Analysis')}</h3>
              <p className="text-sm text-gray-400">{new Date(special.created_at).toLocaleDateString()}</p>
            </div>
            <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ${getStatusColor(special.status)}`}>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {special.ai_response.suggestions?.map((suggestion: string, index: number) => (
                    <div key={index} className="rounded-xl border border-[#2a2a2a] bg-[#2a2a2a]/30 p-3">
              ...
              {special.ai_response.confidence && (
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-gray-400">{Math.round(special.ai_response.confidence * 100)}%</span>
                </div>
              )}
            </div>
          )}
