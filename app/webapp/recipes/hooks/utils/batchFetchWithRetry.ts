...
export async function batchFetchWithRetry(
  ...
  try {
    logger.dev(
      `[RecipeIngredients] Batch fetching for ${recipeIds.length} recipe IDs: ${JSON.stringify(recipeIds)}`,
    );
    ...
    if (!response.ok) {
      ...
      if (response.status >= 500)
        return await handleRetryHelper(
          recipeIds,
          retryCount,
          maxRetries,
          performBatchFetchRef,
          getRetryDelay,
        );
      return {};
    }
    ...
    logger.dev(
      `[RecipeIngredients] Batch fetch completed in ${duration}ms, got ${Object.keys(items).length} recipe groups`,
    );
