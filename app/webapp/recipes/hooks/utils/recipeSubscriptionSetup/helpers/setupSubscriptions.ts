...
export function setupSubscriptionsHelper(refs: SubscriptionRefs, recipes: any[]) {
  ...
  return supabase
    ...
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'ingredients', filter: 'cost_per_unit=neq.null' }, payload => {
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'recipe_ingredients' }, payload => {
      const eventType = payload.eventType;
      const recipeId = eventType === 'DELETE' ? (payload.old as { recipe_id?: string })?.recipe_id : (payload.new as { recipe_id?: string })?.recipe_id || (payload.old as { recipe_id?: string })?.recipe_id;
      logger.dev(`[RecipeSubscriptions] Recipe ingredient ${eventType}:`, { eventType, recipeId });
      if (!recipeId) { logger.warn('[RecipeSubscriptions] No recipe_id in payload', payload); return; }
    ...
    .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'recipes' }, payload => {
      ...
      logger.dev('[RecipeSubscriptions] Recipe metadata updated:', { recipeId, changes: payload.new });
      if (!recipeId) { logger.warn('[RecipeSubscriptions] No recipe ID in payload', payload); return; }
