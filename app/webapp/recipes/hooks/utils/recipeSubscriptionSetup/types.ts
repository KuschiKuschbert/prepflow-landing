...
export interface SubscriptionRefs {
  refreshRecipePricesRef: MutableRefObject<(recipes: Recipe[], fetchRecipeIngredients: (recipeId: string) => Promise<RecipeIngredientWithDetails[]>, fetchBatchRecipeIngredients?: (recipeIds: string[]) => Promise<Record<string, RecipeIngredientWithDetails[]>>) => Promise<void>>;
  fetchRecipeIngredientsRef: MutableRefObject<(recipeId: string) => Promise<RecipeIngredientWithDetails[]>>;
  fetchBatchRecipeIngredientsRef: MutableRefObject<((recipeIds: string[]) => Promise<Record<string, RecipeIngredientWithDetails[]>>) | undefined>;
