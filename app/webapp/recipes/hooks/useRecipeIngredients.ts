...
export function useRecipeIngredients(setError: (error: string) => void) {
  ...
  const fetchBatchRecipeIngredients = useCallback(async (recipeIds: string[]): Promise<Record<string, RecipeIngredientWithDetails[]>> => {
    ...
    if (isProcessingQueue || globalRequestQueue.length > 0) {
      return new Promise<Record<string, RecipeIngredientWithDetails[]>>((resolve, reject) => {
        ...
        if (!isProcessingQueue) processBatchRequestQueue(performBatchFetch).catch(err => logger.error('[RecipeIngredients] Queue processing error:', err));
      });
    }
    const handleQueueError = (err: any) => logger.error('[RecipeIngredients] Queue processing error:', err);
    const batchPromise = performBatchFetch(recipeIds).then(result => { globalBatchRequestCache.delete(cacheKey); processBatchRequestQueue(performBatchFetch).catch(handleQueueError); return result; }).catch(err => { globalBatchRequestCache.delete(cacheKey); processBatchRequestQueue(performBatchFetch).catch(handleQueueError); throw err; });
