  ...
  try {
    ...
    for (const area of cleaningAreas) {
      try {
        ...
        if (error) {
          results.errors.push({ table: 'cleaning_areas', area: area.area_name, error: error.message });
        } else {
          results.populated.push({ table: 'cleaning_areas', action: 'inserted', area: area.area_name });
        }
      } catch (err: any) {
    ...
    for (const supplier of suppliers) {
      try {
        ...
        if (error) {
          results.errors.push({ table: 'suppliers', supplier: supplier.supplier_name, error: error.message });
        } else {
          results.populated.push({ table: 'suppliers', action: 'inserted', supplier: supplier.supplier_name });
        }
      } catch (err: any) {
        results.errors.push({ table: 'suppliers', supplier: supplier.supplier_name, error: err.message });
      }
    }
    ...
    for (const type of complianceTypes) {
      try {
        ...
        if (error) {
          results.errors.push({ table: 'compliance_types', type: type.type_name, error: error.message });
        } else {
          results.populated.push({ table: 'compliance_types', action: 'inserted', type: type.type_name });
        }
      } catch (err: any) {
        results.errors.push({ table: 'compliance_types', type: type.type_name, error: err.message });
      }
    }
    ...
    for (const section of kitchenSections) {
      try {
        ...
        if (error) {
          results.errors.push({ table: 'kitchen_sections', section: section.section_name, error: error.message });
        } else {
          results.populated.push({ table: 'kitchen_sections', action: 'inserted', section: section.section_name });
        }
      } catch (err: any) {
        results.errors.push({ table: 'kitchen_sections', section: section.section_name, error: err.message });
      }
    }
    ...
    for (const equipment of temperatureEquipment) {
      try {
        ...
        if (error) {
          results.errors.push({ table: 'temperature_equipment', equipment: equipment.name, error: error.message });
        } else {
          results.populated.push({ table: 'temperature_equipment', action: 'inserted', equipment: equipment.name });
        }
      } catch (err: any) {
        results.errors.push({ table: 'temperature_equipment', equipment: equipment.name, error: err.message });
      }
    }
