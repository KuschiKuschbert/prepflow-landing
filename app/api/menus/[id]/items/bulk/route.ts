...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Prepare items for insertion with position calculation
    ...
    // Group items by category to calculate positions
    ...
    for (const [category, categoryItems] of Object.entries(categoryGroups)) {
      ...
      for (const item of categoryItems) {
        ...
        if (item.dish_id) {
          insertItem.dish_id = item.dish_id;
        } else if (item.recipe_id) {
          insertItem.recipe_id = item.recipe_id;
        }
