    ...
    if (item.dishes?.id) {
      ...
      if (dietaryResult) {
        ...
        const cachedIsVegan = validateVeganStatus(
          (item.dishes as unknown).is_vegan ?? null,
          allergens,
          'dish',
          item.dishes.id,
          dishName,
        ...
        isVegetarian = (item.dishes as unknown).is_vegetarian ?? null;
        isVegan = cachedIsVegan;
        dietaryConfidence = (item.dishes as unknown).dietary_confidence ?? null;
        dietaryMethod = (item.dishes as unknown).dietary_method ?? null;
      }
    }
  } catch (err: unknown) {
