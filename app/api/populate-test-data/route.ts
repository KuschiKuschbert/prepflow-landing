...
export async function POST() {
  ...
  try {
    ...
    if (dishesError) {
      ...
      dishes?.forEach(dish => {
        if (
          !uniqueDishes.has(dish.name) ||
          new Date(dish.created_at) > new Date(uniqueDishes.get(dish.name).created_at)
        ) {
      ...
      // Delete duplicates
      const duplicateIds =
        dishes?.filter(dish => dish.id !== uniqueDishes.get(dish.name)?.id).map(d => d.id) || [];
    ...
    const cleaningAreas = [
      ...
      {
        area_name: 'Windows & Doors',
        description: 'All windows and entry doors',
        cleaning_frequency: 'Weekly',
      },
      {
        area_name: 'Garbage Area',
        description: 'Garbage bins and waste disposal area',
        cleaning_frequency: 'Daily',
      },
    ];
    ...
    const cleaningTasks = [
      ...
      {
        area: 'Prep Station',
        task_name: 'Sanitize Surfaces',
        description: 'Clean and sanitize all prep surfaces',
        frequency: 'After each use',
        duration: 15,
      },
      {
        area: 'Storage Area',
        task_name: 'Organize Storage',
        description: 'Check dates and organize storage areas',
        frequency: 'Weekly',
        duration: 45,
      },
      {
        ...
        description: 'Wipe down all dining tables and chairs',
        frequency: 'After each service',
        duration: 20,
      },
      {
        area: 'Bathroom',
        task_name: 'Deep Clean',
        description: 'Clean toilets, sinks, and restock supplies',
        frequency: 'Every 2 hours',
        duration: 25,
      },
      {
        area: 'Equipment',
        task_name: 'Sanitize Equipment',
        description: 'Clean and sanitize all cooking equipment',
        frequency: 'After each use',
        duration: 30,
      },
      {
        area: 'Windows & Doors',
        task_name: 'Clean Glass',
        description: 'Clean all windows and door glass',
        frequency: 'Weekly',
        duration: 60,
      },
      {
        area: 'Garbage Area',
        task_name: 'Empty Bins',
        description: 'Empty garbage bins and clean area',
        frequency: 'Daily',
        duration: 15,
      },
    ];
    ...
    for (const task of cleaningTasks) {
      ...
      if (areaId) {
        const { error } = await supabaseAdmin.from('cleaning_tasks').upsert(
          {
            area_id: areaId,
    ...
    const complianceTypes = [
      ...
      {
        type_name: 'Equipment Maintenance',
        description: 'Kitchen equipment maintenance checks',
        frequency: 'Weekly',
      },
      {
        type_name: 'Health Inspection',
        description: 'Council health inspection preparation',
        frequency: 'Quarterly',
      },
      {
        type_name: 'Allergen Management',
        description: 'Allergen control and labeling verification',
        frequency: 'Daily',
      },
    ];
    ...
    for (const section of sections || []) {
      const { error } = await supabaseAdmin.from('prep_lists').upsert(
        {
          prep_date: today,
    ...
    for (const supplier of sampleSuppliers) {
      const { error } = await supabaseAdmin.from('order_lists').upsert(
        {
          order_date: today,
