...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(ApiErrorHandler.createError('Reset is disabled in production', 'FORBIDDEN', 403), { status: 403 });
  }
  ...
  for (const table of tablesInOrder) {
    ...
    if (error) {
      ...
      return NextResponse.json(ApiErrorHandler.createError('Reset failed', 'SERVER_ERROR', 500, results ), { status: 500 });
    }
    results.push({ table });
  }
