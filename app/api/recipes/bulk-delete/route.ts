...
export async function POST(request: NextRequest) {
  try {
    ...
    // Delete recipes
    const { error: recipesError } = await supabaseAdmin.from('recipes').delete().in('id', recipeIds);
    ...
    if (recipesError) {
      ...
      if (
        ...
        return NextResponse.json(
          {
            error: 'Cannot delete recipes',
            message: 'One or more recipes are used in dishes. Please remove them from all dishes first.',
          },
          { status: 400 },
        );
      }
