...
export const createParLevelSchema = z
  .object({
    ...
    par_level: z
      .union([
        z.number().positive('Par level must be a positive number'),
    ...
    reorder_point: z
      .union([
        z.number().positive('Reorder point must be a positive number'),
        z.string().transform(val => parseFloat(val)),
      ])
      .optional(),
  ...
  .refine(
    data => {
      ...
      return (
        !!ingredientId &&
        parLevel !== undefined &&
        !isNaN(Number(parLevel)) &&
        reorderPoint !== undefined &&
        !isNaN(Number(reorderPoint)) &&
        !!data.unit
      );
    },
    {
      message:
        'All fields are required: ingredient_id (or ingredientId), par_level (or parLevel), reorder_point (or reorderPoint), and unit',
    },
  );
