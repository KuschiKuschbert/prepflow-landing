...
describe('processBatchCreation', () => {
  ...
  it('should create prep lists and items successfully', async () => {
    ...
    (supabaseAdmin!.from as jest.Mock).mockReturnValueOnce({ // insert list
    ...
    (supabaseAdmin!.from as jest.Mock).mockReturnValueOnce({ // insert items
    ...
    const prepLists = [
      {
        ...
        name: 'Morning Prep',
        items: [
          { ingredientId: 'ing-1', quantity: '10', unit: 'kg', notes: '' }
        ]
      }
    ];
  ...
  it('should handle partial failures', async () => {
    ...
    (supabaseAdmin!.from as jest.Mock).mockReturnValueOnce({ // insert list 1
        insert: jest.fn().mockReturnValueOnce({
          select: jest.fn().mockReturnValueOnce({
    ...
    (supabaseAdmin!.from as jest.Mock).mockReturnValueOnce({ // insert items 1
        insert: jest.fn().mockResolvedValue({ error: null })
    });

    // List 2: Fails at DB level
    (supabaseAdmin!.from as jest.Mock).mockReturnValueOnce({ // insert list 2
        insert: jest.fn().mockReturnValueOnce({
          select: jest.fn().mockReturnValueOnce({
    ...
    const prepLists = [
      { sectionId: null, name: 'Success List', items: [{ ingredientId: '1', quantity: '1', unit: 'g', notes: '' }] },
      { sectionId: null, name: 'Fail List', items: [{ ingredientId: '2', quantity: '1', unit: 'g', notes: '' }] }
    ];
