...
interface UseAnimationHooksParams {
...
export function useAnimationHooks(params: UseAnimationHooksParams) {
  const {
    displayFeature,
    imageMounted,
    expandedFeature,
  ...
  const imageData = useImageTransitions(displayFeature, imageMounted);
  const containerHeight = useContainerHeight(imageData.imageDimensions, imageContainerRef);
  ...
  const handleToggle = createToggleHandler(expandedIndex, features, containerRefs, parentContainerRef, initialWidths, setExpandedIndex, setContainerWidths, setScaleXValues, setIsTransitioning);

  return { ...imageData, containerHeight, handleToggle };
}
