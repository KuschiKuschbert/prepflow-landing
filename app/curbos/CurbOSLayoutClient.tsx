...
import { ArrowLeft, BarChart3, Cog, LogOut, Monitor, RotateCw, Settings, User, UtensilsCrossed } from 'lucide-react'
  ...
  return (
    <>
      ...
      {!pathname.startsWith('/curbos/order/') && !pathname.startsWith('/curbos/quests/') && (
        <header className="fixed top-4 left-4 right-4 tablet:top-6 tablet:left-6 tablet:right-6 h-16 tablet:h-20 bg-neutral-900/80 backdrop-blur-xl border border-white/10 rounded-2xl z-50 flex items-center gap-2 tablet:gap-4 px-4 tablet:px-6 desktop:px-8 shadow-2xl shadow-black/50">
          <Link href="/curbos" className="flex items-center gap-1.5 tablet:gap-4 group cursor-pointer flex-shrink-0 max-w-[120px] tablet:max-w-fit">
          ...
          <nav className="flex items-center gap-0.5 tablet:gap-1 bg-black/20 p-1 tablet:p-1.5 rounded-xl border border-white/5 overflow-x-auto flex-nowrap scrollbar-hide flex-1 min-w-0 mx-1 tablet:mx-2 desktop:mx-4 shrink">
          ...
          <div className="flex items-center gap-1 tablet:gap-2 desktop:gap-4 flex-shrink-0">
             {/* Back to PrepFlow Button */}
             <Link
               href="/webapp"
               className="flex items-center justify-center text-neutral-400 hover:text-[#C0FF02] transition-colors group px-1 tablet:px-2 py-1 min-h-[44px]"
               aria-label="Back to PrepFlow"
             >
               <ArrowLeft size={16} className="tablet:w-4 tablet:h-4 group-hover:-translate-x-1 transition-transform" />
             </Link>
             ...
             <form action={seedInitialData}>
                <button className="flex items-center gap-1 tablet:gap-2 text-[8px] tablet:text-[10px] text-neutral-600 hover:text-[#C0FF02] uppercase tracking-widest transition-colors font-bold border border-white/5 hover:border-[#C0FF02]/30 px-1.5 tablet:px-3 py-1 tablet:py-1.5 rounded-lg group min-h-[44px]">
             ...
             <button
                ...
                className="flex items-center gap-1 tablet:gap-2 text-neutral-400 hover:text-red-400 transition-colors group px-1 tablet:px-2 py-1 min-h-[44px]"
              >
