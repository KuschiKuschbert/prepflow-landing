...
export async function seedInitialData() {
  try {
    ...
    for (const item of initialItems) {
      const { data, error: selectError } = await supabase.from('pos_menu_items').select('id').eq('name', item.name).maybeSingle()
      if (selectError) {
          logger.error('Error checking item:', { error: selectError.message, itemName: item.name })
          continue
      }
      if (!data) {
          const { error: insertError } = await supabase.from('pos_menu_items').insert([{ ...item, id: crypto.randomUUID() }])
    ...
    for (const mod of initialModifiers) {
      const { data, error: selectError } = await supabase.from('pos_modifier_options').select('id').eq('name', mod.name).maybeSingle()
      if (selectError) {
      ...
      if (!data) {
          ...
          const { error: insertError } = await supabase.from('pos_modifier_options').insert([dbMod])
    ...
    logger.error('Unexpected error in seedInitialData:', { error: error instanceof Error ? error.message : String(error) })
  }
}
